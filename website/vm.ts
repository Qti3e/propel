...
export function createRPCHandler(handler: OutputHandlerDOM | LookupCell) {
  ...
  return {
    plot(cellId: CellId, data: any): any {
      return lookupCell(cellId).plot(data);
    },

    print(cellId: CellId, data: any): any {
      return lookupCell(cellId).print(data);
    },

    imshow(cellId: CellId, data: any): any {
      return lookupCell(cellId).imshow(data);
    },

    downloadProgress(cellId: CellId, data: any): void {
      return lookupCell(cellId).downloadProgress(data);
    }
  };
}
